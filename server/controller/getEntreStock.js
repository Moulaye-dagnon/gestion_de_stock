...
const getEntreStocks = async (req, res) => {
  try {
    const [row] = await pool.execute(
      " SELECT E.id, P.nom as produit, F.nom as fournisseur, E.quantiteEntre , E.dateEntre FROM entrestock E JOIN produit P ON P.id = E.produitId JOIN fournisseur F ON F.id = E.fournisseurId"
    );
    if (row.length == 0)
